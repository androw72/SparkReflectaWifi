...
// Checksum for the outgoing frame, calculated byte by byte using XOR.  Added to the payload as the last byte of the frame. 
  ...
  void writeEscaped(byte b, Adafruit_CC3000_ClientRef& client)
  {
    switch(b)
    {
      case End:
  ...
  void sendEvent(FunctionId type, byte code, Adafruit_CC3000_ClientRef& client)
  {
    byte buffer[2];
    buffer[0] = type;
    buffer[1] = code;
  ...
  int readUnescaped(byte &b, Adafruit_CC3000_ClientRef& client)
  {
    b = client.read();
