...
namespace reflectaFunctions
{
  ...
  void (*vtable[26])(Adafruit_CC3000_ClientRef& client);
  ...
  void run(byte i, Adafruit_CC3000_ClientRef &client)
  {
    if (vtable[i] != NULL)
    {
      vtable[i](client);      
 ...
 int16_t pop16()
  ...
  void sendResponseCount(Adafruit_CC3000_ClientRef& client)
  {
  ...
  void pushArray(Adafruit_CC3000_ClientRef& client)
  {
  ...
  void frameReceived(byte sequence, byte frameLength, byte* frame, Adafruit_CC3000_ClientRef &client)
  {    
  ...
  // interface
  void queryInterface(Adafruit_CC3000_ClientRef &client) {
