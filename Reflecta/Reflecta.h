...
namespace reflectaFrames
{
  ...
  typedef void (*frameReceivedFunction)(byte sequence, byte frameLength, byte* frame, Adafruit_CC3000_ClientRef& client);
  ...
  void sendEvent(reflecta::FunctionId type, byte code, Adafruit_CC3000_ClientRef& client);
  
  // Send a string message, generally used for debugging
  void sendMessage(String message, Adafruit_CC3000_ClientRef& client);
  
  // Send a frame of data returning the sequence id
  byte sendFrame(byte* frame, byte frameLength, Adafruit_CC3000_ClientRef& client);
  ...
  //void loop(Adafruit_CC3000_ClientRef client);
  void loop(Adafruit_CC3000_ClientRef& client);
...
namespace reflectaFunctions
{
  ...
  byte bind(String interfaceId, void (*function)(Adafruit_CC3000_ClientRef& client));
  
  void push(int8_t b);
  void push(int8_t b, Adafruit_CC3000_ClientRef& client);
  
  void push16(int16_t b);
  void push16(int16_t b, Adafruit_CC3000_ClientRef& client);
  
  int8_t pop(Adafruit_CC3000_ClientRef& client);

  int16_t pop16(Adafruit_CC3000_ClientRef& client);
  int16_t pop16();
  ...
  void sendResponse(byte parameterLength, byte* parameters, Adafruit_CC3000_ClientRef& client);
...
namespace reflectaArduinoCore
{
  ...
  //int freeRam ();
  void pinMode(Adafruit_CC3000_ClientRef& client);
  void digitalRead(Adafruit_CC3000_ClientRef& client);
  void digitalWrite(Adafruit_CC3000_ClientRef& client);
  void analogRead(Adafruit_CC3000_ClientRef& client);
  void analogWrite(Adafruit_CC3000_ClientRef& client);
...
namespace reflectaHeartbeat
{
  ...
  void push16(int16_t w, Adafruit_CC3000_ClientRef& client);
  
  void pushf(float f, Adafruit_CC3000_ClientRef& client);
  
  // Bind a function to an interfaceId so it can be called by Reflecta Functions.  The assigned frame id is returned.
  void bind(bool (*function)(Adafruit_CC3000_ClientRef& client));
